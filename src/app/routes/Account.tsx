      ...
      <aside className="bg-white border border-[#ECE6DF] p-6 h-fit">
        ...
        <ul className="space-y-1 text-[12px]">
          {sidebarItems.map(item => (
            <li key={item.key}>
              <button onClick={()=>setTab(item.key)} className={`w-full text-left px-3 py-2 flex justify-between items-center rounded transition ${tab===item.key ? "bg-[#111] text-white font-semibold" : "text-[#5C5651] hover:bg-[#FBF6F0]"}`}>
        ...
        <button onClick={()=>{logout(); nav("/")}} className="mt-4 w-full border border-[#111] h-9 text-[11px] uppercase">{lang==="zh"?"登出":"Logout"}</button>
      </aside>
      ...
      <section className="space-y-6 md:space-y-8">
                  ...
                  <Link to={`/order/${o.id}`} key={o.id} className="border border-[#F2ECE4] p-5 hover:bg-[#FBF6F0] transition block">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`text-[10px] tracking-[0.12em] uppercase px-2 py-[1px] ${o.status==="paid"?"bg-green-100 text-green-700":o.status==="shipped"?"bg-blue-100 text-blue-700":o.status==="delivered"?"bg-[#111] text-white":o.status==="cancelled"?"bg-red-100 text-red-700":"bg-[#F2ECE4] text-[#8F8881]"}`}>{o.status}</span>
                      </div>
                    </div>
        ...
        {tab==="birthday" && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-[#ECE6DF] p-6 text-[12px] leading-relaxed text-[#5C5651]">
              <h4 className="font-semibold text-[13px] text-[#111] mb-3">{lang==="zh"?"生日禮遇條款":"Birthday Rewards Terms"}</h4>
        ...
        {tab==="addresses" && (
          <div className="space-y-4">
                ...
                : <div className="space-y-3">{addresses.map(a => (
                    <div key={a.id} className="border border-[#F2ECE4] p-4 flex justify-between">
                      <div className="text-[12px]">
                        <p className="font-semibold">{a.name} {a.isDefault && <span className="text-[10px] bg-[#111] text-white px-1 ml-1">{lang==="zh"?"預設":"Default"}</span>}</p>
            ...
            {!editingAddress ? (
              <button onClick={()=>setEditingAddress({ name: user.username, phone: "", address: "", district: lang==="zh"?"香港島":"Hong Kong Island" })} className="bg-[#111] text-white px-6 h-10 text-[11px] tracking-[0.14em] uppercase">+ {lang==="zh"?"新增地址":"Add Address"}</button>
            ) : (
              <div className="bg-white border border-[#ECE6DF] p-6 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={addAddress} className="bg-[#111] text-white px-6 h-10 text-[11px] uppercase">{lang==="zh"?"儲存":"Save"}</button>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-[#ECE6DF] p-6">
              ...
              <div className="space-y-3">
                ...
                <button onClick={handleSaveProfile} className="bg-[#111] text-white px-6 h-10 text-[11px] tracking-[0.14em] uppercase">{lang==="zh"?"儲存更改":"Save Changes"}</button>
              </div>
            </div>
            <div className="bg-white border border-[#ECE6DF] p-6">
            ...
            <div className="bg-white border border-[#ECE6DF] p-6">
              ...
              <div className="space-y-3">
                ...
                <button onClick={()=>showToast("info", lang==="zh"?"密碼更改功能即將推出":"Password change coming soon")} className="bg-[#111] text-white px-6 h-10 text-[11px] tracking-[0.14em] uppercase">{lang==="zh"?"更改密碼":"Change Password"}</button>
              </div>
            </div>
          </div>
        )}
